...
# 1. Single ECS/Fargate execution mode
# 2. Future parallel suite execution mode
FLAKY_REPORT_FOLDERS = {
    "Single ECS Run": "flaky-reports/flaky_report.json",
...
def build_dashboard_data():
    ...
    for suite_name, report_path in FLAKY_REPORT_FOLDERS.items():
        ...
        # Skip empty reports that do not exist or have no executed tests
        if report.get("total_tests", 0) == 0 and report.get("retried_tests", 0) == 0:
            continue
...
def create_text_summary(dashboard):
    ...
    # Add suite summary
    if not dashboard["suites"]:
        lines.append("No flaky report data found.")
...
def write_allure_dashboard(dashboard):
    ...
    allure_result = {
        ...
        "stage": "finished",
        "description": "Aggregated flaky test analytics generated from retry tracking reports.",
        "labels": [
